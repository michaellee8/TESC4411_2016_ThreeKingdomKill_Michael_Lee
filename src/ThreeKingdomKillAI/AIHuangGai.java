...
public class AIHuangGai extends AICoreHuangGai {
	...
	@Override
	public void beingAttack() {
		if (this.canDefense() && !(((this.countHandCards(Card.Heal) > 0) || this.countHandCards(Card.Vitality) > 0)
				&& this.getHp() - 100 > this.getMaxHp() / 2)) {
			use(Card.Defense);
		}
	}
	...
	protected void beforeMyTurn() {
		i = 0;
		while (i <= 5 && this.countHandCards(Card.Heal) > 0 && this.getHp() <= this.getMaxHp() / 2) {
			this.use(Card.Heal);
			i++;
		}
		...
		i = 0;
		while (i <= 5 && (this.countHandCards(Card.Heal) > 0 || this.countHandCards(Card.Vitality) > 0)
				&& this.getHp() - 75 >= this.getMaxHp() / 2 && this.countHandCards() < 8) {
			this.spec();
